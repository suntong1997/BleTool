...
public class HistoryHeartRate {
    private static Calendar calendar = Calendar.getInstance(Locale.CHINA);

    @SuppressLint("ResourceType")
    public static void onRequestHeartRateOfDay(Context context, Iview iView, BluetoothLeService bluetoothLeService, String deviceAddress) {
        ...
        new DatePickerDialog(
                ...
                (view, year, monthOfYear, dayOfMonth) -> {
                    ...
                    if (intervalDay == 0) { // 判断选择的时间是否为今天，若是则结束传入当前时间作为结束时间
                        ...
                        HR_DATA_CMD = new byte[]{0x01, 0x02, 0x07, 0x00, intervalDay, 0x00, 0x017};
                    }
                    bluetoothLeService.writeCharacteristic(deviceAddress, HR_DATA_CMD);
                    // 重置读取获取多包指令条件
//                    recivePkgNum = 0;
//                    isReciveMultPkg = false;
